...
if len(sys.argv) != 4:
    print "COLORING_CLINGEN.py <PARAMETERFILES PATH> CLINGEN <R Command>"
    sys.exit(1)
...
try:
 ...
 for rf in FILES:
    ...
    for coder in RCODE:
        ...
        # Change plot title (DB_GAIN or DB_LOSS)
            ...
            LINERCODE = rcode.strip().split("'")
            RCODED.append(LINERCODE[0].strip()+"'"+sys.argv[2]+' '+rf.strip().split('/')[-1].split('_')[-1].split('.')[0]+" ("+GENOME+"): '"+LINERCODE[2].strip())

        else: RCODED.append(rcode.strip())
    RCODE.close();

    # Write Changes to File
    OFILE=open(PATH+'/HEATMAP_R_CODE_CLINGEN.R','w');
    for rcodl in RCODED:
        OFILE.write(rcodl.strip()+'\n');
    OFILE.close();

    # Run HEATMAP_R_CODE.R
    COMMAND = RVERSION +' '+ 'CMD BATCH' +' '+ PATH + '/HEATMAP_R_CODE_CLINGEN.R'
    os.system(COMMAND);
 ...
 for u in range(KL,len(L)):
    if os.path.exists('/'.join(['']+L[0:KL]).strip()+'/HEATMAP_R_CODE_CLINGEN.Rout'):
        COMMAND='rm '+'/'.join(['']+L[0:KL]).strip()+'/HEATMAP_R_CODE_CLINGEN.Rout'
        os.system(COMMAND);
        break;
    else:
        KL+=1;
